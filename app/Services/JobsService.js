...
class JobsService {
  ...
  async addJob(formData) {
    const response = await SandboxServer.post("/api/jobs", formData)
  ...
  async editJob(formData) {
    ...
    const response = await SandboxServer.put(`/api/jobs/${activeJob.id}`, formData)
  ...
  async deleteJob(jobId) {
    await SandboxServer.delete(`/api/jobs/${jobId}`)
